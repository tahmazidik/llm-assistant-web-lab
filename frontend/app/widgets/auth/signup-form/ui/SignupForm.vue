<script setup lang="ts">
import { ref } from 'vue'
import { useRuntimeConfig, navigateTo } from "#app"

const name = ref('')
const email = ref('')
const password = ref('')
const pending = ref(false)
const errorMessage = ref<string | null>(null)

interface RegisterResponse {
  id: string
  email: string
  name: string
  create_at: string
  update_at: string
}

interface LoginResponse {
  user: {
    id: string
    email: string
    name: string
    create_at: string
    update_at: string
  }
  token: string
}

const handleSubmit = async () => {
  errorMessage.value = null
  pending.value = true

  try {
    const config = useRuntimeConfig()
    const apiBase = (config.public.apiBase as string | undefined) || 'http://localhost:8080'

    const res = await $fetch<RegisterResponse>(`${apiBase}/users/register`, {
      method: 'POST',
      body: {
        email: email.value,
        password: password.value,
        name: name.value,
      },
    })

    console.log('Signup success:', res)

    const loginRes = await $fetch<LoginResponse>(`${apiBase}/users/login`, {
      method: 'POST',
      body: {
        email: email.value,
        password: password.value,
      },
    })

    localStorage.setItem('authToken', loginRes.token)
    localStorage.setItem('authUser', JSON.stringify(loginRes.user))

    await navigateTo('/')

    await navigateTo('/login')
  } catch (err: any) {
    errorMessage.value = err?.data?.error || 'Не удалось зарегистрироваться. Попробуй еще раз.'
  } finally {
    pending.value = false
  }
}
</script>

<template>
  <div class="min-h-screen bg-[#292828] text-slate-200 flex items-center justify-center px-4">
    <div class="w-full max-w-md bg-[#202123] border border-[#4c3d42] rounded-3xl p-6 md:p-8 shadow-[0_18px_50px_rgba(0, 0, 0, 0.5)]">
      <h1 class="text-2xl md:text-3xl font-semibold text-center mb-2">
        Sign up
      </h1>

      <p class="text-center text-sm text-slate-400 mb-6">
        Create an account to start using LLM Assistant
      </p>

      <form class="space-y-4" @submit.prevent="handleSubmit">
        <div class="space-y-1">
          <label class="block text-sm text-slate-300" for="name">
            Name
          </label>

          <input
            id="name"
            v-model="name"
            type="text"
            required
            class="w-full bg-[#18181b] border border-[#3c3d42] rounded-xl px-3 py-2.5
                  text-sm text-slate-200 placeholder:text-slate-500 focus:border-slate-300 outline-none transition"
            placeholder="Enter your name"
          >
        </div>

        <div class="space-y-1">
          <label class="block text-sm text-slate-300" for="email">
            Email
          </label>

          <input
            id="email"
            v-model="email"
            type="email"
            required
            class="w-full bg-[#18181b] border border-[#3c3d42] rounded-xl px-3 py-2.5
                  text-sm text-slate-200 placeholder:text-slate-500 focus:border-slate-300 outline-none transition"
            placeholder="Enter your email"
          >
        </div>

        <div class="space-y-1">
          <label class="block text-sm text-slate-300" for="password">
            Password
          </label>

          <input
            id="password"
            v-model="password"
            type="password"
            required
            class="w-full bg-[#18181b] border border-[#3c3d42] rounded-xl px-3 py-2.5
                  text-sm text-slate-200 placeholder:text-slate-500 focus:border-slate-300 outline-none transition"
            placeholder="••••••••"
          >
        </div>

        <p v-if="errorMessage" class="text-sm text-red-400">
          {{ errorMessage }}
        </p>

        <button
          type="submit"
          class="w-full mt-2 inline-flex items-center justify-center rounded-xl
                bg-white text-black font-medium px-4 py-2.5
                 hover:bg-slate-200 transition disabled:opacity-50 disabled:cursor-not-allowed"
          :disabled="pending"
        >
          <span v-if="!pending"> Create account </span>
          <span v-else> Creating... </span>
        </button>
      </form>

      <p class="mt-4 text-center text-xs text-slate-400">
        Already have an account?
        <NuxtLink to="/login" class="text-slate-100 hover:underline">
          Log in
        </NuxtLink>
      </p>
    </div>
  </div>
</template>

<style scoped lang="scss">

</style>